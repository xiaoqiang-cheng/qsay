...
在当前 Qwen3-0.6B Q4_K_M + llama.cpp 上，用四条 Git 分支规则做了最小对照：

| 请求 | 通用 Prompt | 加 Git 规则 |
|---|---|---|
| 切换分支 | 错误生成 `git branch` | 能提出缺少目标分支，但同时生成了无效候选命令 |
| 切换分支到 develop | 错误生成 `git branch develop` | 正确生成 `git switch develop` |
| 新建 develop 分支并切换 | 错误假设远端分支 | 命令正确，但错误填入 clarification |
| 查看所有分支 | `git branch` | 更明确的 `git branch --list` |

结论：短 Prompt 规则能显著修复已知意图，是适合做回归和模型能力探针的低成本优化；但 0.6B 模型仍会让相互矛盾的字段同时出现，不能靠不断追加命令知识来承担最终正确性。长工具目录还会增加 prompt processing、挤占上下文并产生规则干扰。生产方案仍应让模型输出工具与参数，由 Adapter 渲染命令。

又用同一 33 条用例做了仅改变 Prompt 的完整 A/B。严格 Prompt 明确要求先识别否定、无关、危险根路径和缺参，再允许工具调用：

| Prompt | 工具 exact | 正常参数 exact | critical false-call | p50 / p95 |
|---|---:|---:|---:|---:|
| 当前短 Prompt | 72.7% | 94.7% | 4 | 1849 / 1963ms |
| 严格证据/拒绝 Prompt | 75.8% | 94.7% | 4 | 2144 / 2466ms |

严格 Prompt 修复了少数样本，但错误迁移到了其他无关、否定和危险请求，且延迟上升。因此 Prompt 优化应继续做，但定位是减少常见错误和改善模型输入，不是最终安全或正确性机制。

### 6.2 本机 `-h` 检索对照

对 Git 分支案例执行了受控的 `git switch -h`，本机帮助进程约 11ms、输出 1266 字符。将帮助全文加入 Qwen3-0.6B 的第二次规划后：
...
离线不再是高于质量的硬约束。Provider 选择应以同一评估集的语义正确率、critical false-call、端到端命令验证率和 p95 延迟排序：

1. 默认使用达到质量门槛且端到端最快的非 thinking API 小模型。
2. Qwen3-0.6B 作为完全离线和隐私模式，并继续做领域微调/约束输出。
3. 两条路径必须共享工具 Schema、Adapter、安全策略和报告，不能维护两套命令知识。
4. API 也不直出最终 Shell；更强模型降低路由错误，Adapter 负责确定性落地。
